...
* Toma TODAS las mercancias con estatus 1
	*/
	...
	contenido +=	"<img alt='' src='media/img/dogsad03.jpg'style='height: 256px;' /><br>";
        contenido +=	"<span class='text-danger font-weight-bold'>Por elmomento, no tienes productos en tu catálogo.<span>";
	contenido +=	"</td>";
	contenido +=	"<tr>";
	...
	contenido += "<tr>";
        contenido += "<td> <button onclick='modifyMer(" + i + ")'class='btn btn-warning'>Modificar</button></td>";
	contenido += "<td> <button onclick='deleteMer(" + i + ")'class='btn btn-danger mx-4'>Eliminar</button></td>";
	contenido += "<td><img id='imgFoto' width='128' height='128' alt='' "+"src='data:image/png;base64," + mercancias[i].producto.foto + "' /></td>";
	...
	contenido += "<td>" +	mercancias[i].marca + "</td>";
	contenido += "<td>" +	mercancias[i].modelo + "</td>";
	contenido += "<td>" +	mercancias[i].producto.existencias +"</td>";	
	contenido += "<td>" +	mercancias[i].codigoBarras + "</td>";
	...
	$("#tBodyMercancias").html(contenido);
        ...
        function listMerInactive()
       {
      var contenido = "";

	$.ajax({
	type: "GET",
	url: "rest/mercancia/getAllInactive",
	async: true
	}).done(
	function (data)	
	{	
	...
	contenido += "<tr>";
        contenido += "<td> <button onclick='modifyMer(" + i + ")'class='btn btn-warning'>Modificar</button></td>";
	contenido += "<td> <button onclick='reactivate(" + i + ")'class='btn btn-primary mx-4'>Reactivar</button></td>";
	contenido += "<td><img id='imgFoto' width='128' height='128' alt='' "+"src='data:image/png;base64," + mercancias[i].producto.foto + "' /></td>";
        contenido += "<td>" + mercancias[i].producto.nombre + "</td>";
	contenido += "<td>" + mercancias[i].producto.precioCompra * 2 +"</td>";	
...
81	* @param {int} i contiene el indice de mercancias que se quiere afectar (la posición del registro)

	*/
	function modifyMer(i)
	{
	var objM = mercancias[i];
	$("#imgFoto").prop('src', 'data:image/png;base64,' + objM.producto.foto);
	$("#txaBase64").val(objM.producto.foto);
	$("#txtIdProducto").val(objM.producto.idProducto);
	$("#txtExistencias").val(objM.producto.existencias);
	$("#txtPrecioCompra").val(objM.producto.precioCompra);
	$("#txtPrecioVenta").val(objM.producto.precioCompra * 2);
	$("#txtNombre").val(objM.producto.nombre);
	$("#txtIdMercancia").val(objM.idMercancia);
	$("#txtaDescripcion").val(objM.descripcion);
	$("#txtMarca").val(objM.marca);
	$("#txtModelo").val(objM.modelo);
	$("#txtCodigoBarras").val(objM.codigoBarras);
	}

	/*
101	* Esta función toma la posición del registro en mercancias para
	* tomar su idProducto y poder eliminarlo invocando al servicio para ello
	* @param {int} i contiene el indice de mercancias que se quiere eliminar
	*/
	function deleteMer(i)
	{
	var id = mercancias[i].producto.idProducto;
...
193	*/
	function saveMer()
	{
	var foto = document.getElementById("txaBase64").value;
	...
	var idMercancia = document.getElementById("txtIdMercancia").value;
	...
	{
	Swal.fire("Mercancia almacenada existosamente",
	"Inserción",
	"success");
	...
	/*
	* función que carga la foto seleccionada y devuelve su base64 en un textArea
	*/
	function loadFoto()
	{
	...
	/*
	* Función que limpia los campos de insertar/modificar mercancia
	*/
	function cleanFieldsMer()
	{
	$("#imgFoto").prop('src', 'data:image/png;base64,');
	$("#txaBase64").val("");
	$("#txtIdProducto").val("");
	$("#txtIdMercancia").val("");
	$("#txtCodigoBarras").val("");
...
$("#txtExistencias").val("");
	...
	{
	Swal.fire("Mercancia encontrada existosamente",
	"EXITO",
	"success");                     
